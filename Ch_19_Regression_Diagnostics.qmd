...
This is also the cost that the [Multiple Regression: Statistical Control chapter](Ch_13_Multiple_Regression.qmd) promised you would meet later, where the controlled slope came with a slightly larger standard error than the uncontrolled one. This is later, and this is the meter.
...
If residuals share a source, use a design-aware method such as a mixed-effects model. The [mixed regression chapter](Ch_18_Mixed_Regression.qmd) does exactly that: it tells the model which rows belong to the same person, instead of letting it believe it met a room full of strangers.
