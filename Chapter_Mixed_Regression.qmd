...
<!-- ---Alex--- This cold open is new (CH18-U02) and it replaced the old opening, which was
the "Advanced Chapter" callout plus a recap paragraph. You approved rebuilding it against
VOICE_GUIDE 15. The device is the identity payoff from 17: the paired t-test and the lmer
give the same t, df, p and estimate, verified to six significant digits before any of this
prose was written. The old callout's definition and the "all its relatives / provide chairs"
joke both survive, in the callout at the end of this section. -->
...
After it there is an Advanced Topics break for missing occasions and model checking, then a short section on what three or more occasions require and why this chapter cannot give it to you.
...
That $u_{0i}$ is the same `(1 | participant)` that reproduced the paired *t*-test at the top of the chapter, doing the same job on a bigger design.
...
`lmer()` fits with REML by default, which is what we want while we are looking at the random effects. That default becomes important, and gets deliberately switched off, when we start comparing fixed effects.

::: {.callout-warning title="With Two Occasions, a Random Slope Is Not Available"}
The obvious next thought is to let people change at different *rates*, not just start at different levels, by writing `(1 + week | participant)`. Try it on this data and `lmer` refuses:
...
When comparing **fixed effects**, refit both with maximum likelihood. This is the moment `lmer()`'s REML default has to be switched off by hand, in both models, or the comparison is not a legal one.
