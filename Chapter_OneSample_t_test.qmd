...
kids.1<-sample+rnorm(n,mean=2,sd=0)
...
Children who received cookies showed higher hyperactivity scores ($M =$ `r round(mean(kids.1), 2)`, $SD =$ `r round(sd(kids.1), 2)`) than the hypothesized population value ($\mu = 50$), $t(35) =$ `r round(unname(Kids.t.test$statistic), 2)`, $p$ `r ifelse(Kids.t.test$p.value < .001, "< .001", paste("=", round(Kids.t.test$p.value, 3)))`, 95% CI [`r round(Kids.t.test$conf.int[1], 2)`, `r round(Kids.t.test$conf.int[2], 2)`], $d =$ `r round((mean(kids.1) - 50) / sd(kids.1), 2)`. The standardized difference describes magnitude; whether the effect is practically meaningful requires context and better evidence than our imaginary cookie operation.
