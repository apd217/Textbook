# Chapter 6 — One Sample t-Test (Ch_06_One_Sample_t.qmd)
...
- **Effort:** S
- **Location:** Ch_06_One_Sample_t.qmd → "One-sample t-test in APA format" (search anchor: "#apa(Kids.t.test)")
...
- **Perspective:** Professor + Student
- **Effort:** S
- **Location:** Ch_06_One_Sample_t.qmd → "Test Statistic Result" (search anchor: "we can see our exact p-value which is well below")
...
- **Perspective:** Student
- **Effort:** S
- **Location:** Ch_06_One_Sample_t.qmd → "Non-directional Hypothesis" (search anchor: "Chocolate chip cookies do not change children's level")
...
- **Perspective:** Student
- **Effort:** S
- **Location:** Ch_06_One_Sample_t.qmd → "Type I Error" (search anchor: "Testing zinc or ColdEase on colds")
...
- **Effort:** S
- **Location:** Ch_06_One_Sample_t.qmd → "Make a Decision" (search anchor: "the treatment has *likely* caused a difference")
...
- **Approved:** [x] — IMPLEMENTED 2026-08-14. The empty box was an oversight; Alex approved it verbally in session on 2026-08-14 ("oh that was an error, fix that as well"). Reworded as drafted: the reject-the-null bullet now says the sample mean is farther from the hypothesized value than random sampling error can comfortably explain, and that whether the *cookies* did it is a design question rather than a statistics question, pointing back at the Important Note. "(See Bayesian Statistics)" now points at the Bayes section of the Probability chapter, which is where that material actually lives in the active book. Drafted text used verbatim except the em-dash, which became a full stop per Alex's standing request.

### [CH06-P04] Code hygiene: object named `sample`, constant added via `rnorm(n, mean=2, sd=0)`, stray `var.equal` in a one-sample test
- **Priority:** MED
- **Perspective:** Professor
- **Effort:** S
- **Location:** Ch_06_One_Sample_t.qmd → data-generation chunk (search anchor: "kids.1<-sample+rnorm(n,mean=2,sd=0)")
...
- **Effort:** M
- **Location:** Ch_06_One_Sample_t.qmd → multiple (search anchors: "### State the Hypothesis" [empty stub directly above "### The Null Hypothesis"], "### Sample Experiment", "Estimation of Variance:  we will never know")
...
- **Effort:** S
- **Location:** Ch_06_One_Sample_t.qmd → multiple (search anchors below)
- **Problem / Fix (list):**
...
- **Perspective:** Professor
- **Effort:** S
- **Location:** Ch_06_One_Sample_t.qmd → "Effect Sizes" (search anchor: "created during the invention of the radio and radar")
...
- **Perspective:** Professor
- **Effort:** S
- **Location:** Ch_06_One_Sample_t.qmd → "Cohen's $d$" (search anchor: "d = \\frac{M_{H1}-\\mu_{H0}}{S_{H1}}")
