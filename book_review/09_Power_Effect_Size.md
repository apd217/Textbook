# Chapter 9 — Power and Effect Size (Ch_07_Power_Effect_Size.qmd)
...
- **Effort:** M
- **Location:** Ch_07_Power_Effect_Size.qmd → "The Answer Is Unpleasant" (search anchor: "Our samples were small. With only thirteen children")
...
- **Approved:** [ x] — IMPLEMENTED 2026-08-14 using **option (b)**, the cleaner demo. The old chunk took the SD of 10 study means and then blamed the wobble on the 13 children; it now runs 5,000 studies of 13 children each and plots the SD each study reports against the true $\sigma = 5$. One count, and the fat-middle explanation is now correct as written. The prose names both wobbles and says which one matters. A bonus fell out of it: the simulated mean $S$ is 4.893 and the closed form gives 4.897, so the section now validates the simulation against the theory, which is this chapter's own best habit. Chunk switched to `echo = TRUE`, since it is now three lines and worth reading.

### [CH09-P02] Homer's episode is S2:E2, not S7:E2
- **Priority:** LOW
- **Perspective:** Publisher (fact-check)
- **Effort:** S
- **Location:** Ch_07_Power_Effect_Size.qmd → Homer Simpson callout (search anchor: "Simpson and Delilah")
...
- **Approved:** [ x] — IMPLEMENTED 2026-08-14.

### [CH09-P03] "Radio and radar" again — same fix as CH06-P05
- **Priority:** LOW
- **Perspective:** Professor
- **Effort:** S
- **Location:** Ch_07_Power_Effect_Size.qmd → "Signal Detection Theory" (search anchor: "developed alongside radio and radar")
...
- **Approved:** [ ] — IMPLEMENTED 2026-08-14 **despite the unchecked box**, under Alex's instruction to "wrap up all the lower priority changes" in Part 1. This is the twin of CH06-P05, which was approved and implemented, so leaving it alone would have had Ch6 saying WWII radar while Ch9 said radio, four chapters apart, about the same origin story. Both chapters now use the same wording. Trivially revertible if that was not the intent.

### [CH09-U01] Give the Week-10 student a roadmap — the exam material starts a third of the way in
- **Priority:** LOW
- **Perspective:** Student
- **Effort:** S
- **Location:** Ch_07_Power_Effect_Size.qmd → after the opening callout (search anchor: "## Probability Is Fickle")
...
- **Approved:** [ x] — IMPLEMENTED 2026-08-14 as a `callout-tip` sitting directly under the hidden-code callout, so it is the first thing the reader meets. Names the three altitudes and, more usefully, tells a time-pressed reader to start at "Type I and Type II Error" and read forward. Framed around the 242 exam, since Part 1 serves that course.

### [CH09-B01] Duplicated SDT/Cohen's-d block with Chapter 6 — decide who owns it
- **Priority:** LOW
- **Perspective:** Publisher
- **Effort:** M
- **Location:** Ch_07_Power_Effect_Size.qmd → "Signal Detection Theory" through "Cohen's d" (search anchor: "The hearing-test version of the table above")
...
- **Approved:** [ x] — IMPLEMENTED 2026-08-14, partially. **Cut from this chapter:** the duplicated signal-detection figure (identical to Ch6's), the $\sigma$-based $d$ formula, and the equal-spread formula block, all of which Chapter 6 owns. Replaced with a one-line statement of what $d$ is plus a cross-reference. **Kept and strengthened:** the hearing-test 2×2 mapped onto the power 2×2, which is this chapter's own insight, now with a closing line on why both fields drew the same table. **Deviation from the item:** it asked for the benchmarks table to get a single home. I kept the four-row table in *both* chapters. Ch6 needs it at first introduction, where the reader first asks whether a $d$ is big and where the four visualisations live; Ch9 needs it as a lookup, because power calculations take $d$ as an input and flipping back three chapters mid-calculation is worse than a repeated four-row table. The padding this item was aimed at was the figure and the derivation, and those are gone. Alex's call if you still want it cut from one side.

---

**Also done in this pass (approved global items):**

- **G01** (YAML strip): reduced to `title:` only.
- **G02** (fig-alt): all eight figures now carry `fig-cap` and `fig-alt`, and their `fig.width`/`fig.height` moved from chunk headers into `#|` options.

**Em-dash discipline:** no em-dashes introduced in any new prose.
