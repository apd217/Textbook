# Chapter 10 — Covariance and Correlation (Ch_10_Covariance_Correlation.qmd)
...
- **Effort:** S
- **Location:** Ch_10_Covariance_Correlation.qmd → "No Relationship" (search anchor: "A = c(2,5,3,1,4)")
...
- **Perspective:** Student
- **Effort:** M
- **Location:** Ch_10_Covariance_Correlation.qmd → "Identical Variables" and following sections (search anchors: "Why is it the same number?", "- What does this mean?", "Variances haven't changed?")
...
- **Perspective:** Student
- **Effort:** S
- **Location:** Ch_10_Covariance_Correlation.qmd → "Correlations" (search anchor: "Universal small, medium, and large labels are rarely useful.")
...
- **Effort:** S
- **Location:** Ch_10_Covariance_Correlation.qmd → "Covariance Review" (search anchor: "When the variances of X and Y aren't equal, $Cov(xy)=\\sqrt{Var(x) * Var(y)}$")
...
- **Effort:** M
- **Location:** Ch_10_Covariance_Correlation.qmd → after "Pearson's correlation is scale-independent!" (search anchor: "the correlation will change.")
...
- **Perspective:** Student
- **Effort:** S
- **Location:** Ch_10_Covariance_Correlation.qmd → "Simulate Data" (search anchor: "We will use the `mvrnorm` function")
...
- **Effort:** S
- **Location:** Ch_10_Covariance_Correlation.qmd → multiple
- **Problem / Fix (list):**
...
- **Effort:** M
- **Location:** Ch_10_Covariance_Correlation.qmd → "Variance" through "Covariance Review" (search anchor: "- Variance is our primary measure of spread.")
...
- **Perspective:** Professor
- **Effort:** S
- **Location:** Ch_10_Covariance_Correlation.qmd → Spearman chunks (search anchor: "method = c(\"spearman\"), exact =TRUE")
...
The book PDF fails with `Unable to load picture or PDF file 'Ch_10_Covariance_Correlation_files/figure-pdf/...'`. Cause: `*_files/` is gitignored, and for the two chapters whose *filenames contain spaces* the freeze cache does not preserve `figure-pdf`. Quarto rewrites `Ch_10_Covariance_Correlation` to `Ch_10_Covariance_Correlation` for processing, and the supporting-files bookkeeping does not survive that rename, so nothing gets restored and LaTeX finds no figures. Compare `_freeze/Ch_15_Statistical_Control/execute-results/tex.json`, whose `supporting` entry is `Ch_15_Statistical_Control_files\figure-pdf` *and whose figures are stored*, against this chapter's, which names a directory that was never saved.

Confirmed pre-existing: with all of these Chapter 10 edits stashed, the book still fails to build to PDF, at `Ch_12_Categorical_Regression` instead. Those are the only two chapter filenames containing spaces, and they are the only two that fail. It works on Alex's machine because the gitignored `*_files/` directories are still sitting there from earlier renders.
