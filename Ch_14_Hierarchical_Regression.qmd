...
<!-- ---Alex--- Only change in this finished chapter: the simulation was include=FALSE and
     is now echoed, per your "show them the simulation in all chapters" rule. This was the
     ONLY hidden data-making chunk left in the whole book; everything else was already
     visible. I gave it a lead-in line so it does not hit the reader as a bare wall of code
     immediately before the cold open. If you would rather protect the cold open and leave
     this one hidden, revert this hunk; it is self-contained. -->
...
staircase |>
  ...
  labs(
    ...
    x = "Random predictors added to the real model",
...
The rising line is $R^2$, the proportion of exam-score variance the model explains. It starts with one real predictor, Study Hours, at `r round(staircase$R2[1], 3)`. Then I add random variables one at a time. Ten in all, and every single one leaves the model fitting better than it was before.

I did not measure anything as the variables were just made by `rnorm()`---pure Gaussian noise. They do not know what an exam is, they have never met a student, and each one existed for roughly a millisecond before being hired as a predictor for this model. $R^2$ climbed to `r round(staircase$R2[11], 3)` anyway, because $R^2$ is not permitted to fall below that of the initial real model. Every predictor you add hands least squares one more knob, and least squares will always find some small turn of that knob that fits *this particular sample* a little "more better". *Does this worry you? It should.* 

Here is the part that should worry you even more. A real predictor with real theory behind it raises $R^2$ by `r round(anxiety_gain, 3)`. Ten columns of pure noise raised it by `r round(staircase$R2[11] - staircase$R2[1], 3)`. Measured in raw $R^2$, garbage and theory bought roughly the same thing.

The orange line is the first hint of a defense against this problem. **Adjusted $R^2$** charges more rent for every predictor you take on, and by the tenth noise column it has sagged from `r round(staircase$AdjR2[1], 3)` to `r round(staircase$AdjR2[11], 3)`, below where it started. That is useful. It is also only a rule of thumb. It can tell you something smells wrong. It cannot tell you whether to believe a result.
...
<!-- ---Alex--- Your edit dropped "sequential" as a synonym, which left this sentence saying
     hierarchical regression is also called hierarchical regression. Rewritten so the callout
     still does its job. If you want "sequential" gone from the book entirely, say so and I
     will sweep the other uses. -->
What we are doing here is adding predictors in theory-driven steps. It has nothing to do with **hierarchical linear models**, which are multilevel or mixed models for clustered data: students inside classrooms, trials inside participants, patients inside hospitals. Those get their own chapter, [Mixed Regression](Ch_18_Mixed_Regression.qmd).
...
<!-- ---Alex--- "R^2" outside math renders as a literal caret, "R^2", in both HTML and PDF.
     Pandoc needs "R^2^" or math. Switched to $R^2$, which is what Intro_to_regression and GLM
     already use in their headings. Chapter 13 has three headings with the same bug
     (lines 329, 389, 395); say the word and I will fix those too. -->
## The Change in $R^2$: What Are We Measuring?
...
Not one of those five thousand variables knows anything about exam scores. The average one still bought `r round(mean(null_delta) * 100, 1)`% of the variance, and the luckiest of the five thousand bought `r round(max(null_delta) * 100, 1)`%. Nothing is never worth exactly nothing.
...
### Remember: This *F* Is a *t* You Have Already Met
