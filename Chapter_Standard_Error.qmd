...
### Traveling on the CTA in the Lap of Luxury 
...
### Randomly Sample from the Population, Which Is Harder Than It Sounds
...
#| fig-alt: "Histogram of 80 sample means, each from a sample of ten riders. The means form a roughly normal-shaped pile centered near 3, spanning from about 2.2 to about 3.8."
...
3, and it was precisely: Mean of means = `r round(mean(Sample.Means),2)`. *Remember, you would not ever know the true population mean because (a) good luck getting every single rider of the CTA to answer a survey question, and more importantly (b) the Probability Gods only love me and not you, cause you have not spent your career tricking undergrads into jumping into a volcano.*
...
Thus, the population mean is hopefully between: `r round(M.S-2*SD.S,2)` and `r round(M.S+2*SD.S,2)`

(*Assuming random sampling and a roughly normal sampling distribution---both of which we’re getting for free here.*)

::: {.callout-warning title="Two Is a Shortcut for Now; We Return to It Next Chapter"}
...
For example, with $N=10$, the 95% multiplier is about 2.26---not exactly 2. We will learn where that number comes from in the t-test chapter. For now, $\pm2SE$ is a useful approximation. But **hold your Guinness Beers** (*that's dramatic foreshadowing btw---see, English teachers, I learned something between all those Fs for my awesome essays about penguins.*)
:::

### The Standard Error of the Mean (SEM), the One Number to Rule Them All
...
In other words: the math mostly agrees with the coerced labor. *Now I can pay my loans and buy a convertible Porsche like other middle-aged men and look ridiculous with my bald head reflecting the sun into other people's aghast faces.*
...
So look at that, my error drops really close to zero pretty quickly. Past about 400 the curve is basically crawling: going all the way out to 10,000 riders buys me a standard error of `r round(sd(CTA.Population)/sqrt(10000), 3)` instead of `r round(sd(CTA.Population)/sqrt(400), 3)`. 
...
<!-- ALEX: this sentence had lost its own punchline -- it read "I told you in the last
     chapter that truth is, but in reality that means will never actually know..." I
     restored the G'Kar quote from Ch4 ("truth is a river and god is the mouth of that
     river"), since that is what makes the God bullets below land. Check I guessed right. -->

I told you in the last chapter that truth is a river and god is the mouth of that river. In reality that means you will **never** actually know the true population value unless you are either
...
Yes, but all of this stuff (SDs, SEs, sampling distributions, confidence intervals) exists for one reason: *to let us draw inferences about a population using imperfect data.*
