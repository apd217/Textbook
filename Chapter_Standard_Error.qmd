...
set.seed(1)
SampleSize = 10
sample.1<-sample(CTA.Population, SampleSize, replace = TRUE)
...
set.seed(26)
SampleSize = 10
sample.2<-sample(CTA.Population, SampleSize, replace = TRUE)
...
NumberofSamples=80
SampleSize=10
Sample.Results<-replicate(NumberofSamples,sample(CTA.Population, size = SampleSize, replace=TRUE))
...
$$S_M = \sqrt{\frac{\Sigma (M-M_{Grand})^2}{N-1}}$$
...
ggplot(sem_df, aes(x = N, y = SEM)) +
  geom_line(linewidth = 1) +
  coord_cartesian(ylim = c(0, .2), xlim=c(10, 400)) +
  labs(
    title = "Standard Error of the \nMean as a Function of Sample Size",
    x = "Sample Size (N)",
    y = "Standard Error of the Mean (SEM)"
...
set.seed(26)
SampleSize = 100
sample.3<-sample(CTA.Population, SampleSize, replace = TRUE)
M3<-mean(sample.3)
SD3<-sd(sample.3)
SEM3<-sd(sample.3) / sqrt(100)
...
- $S_M = \frac{.957}{\sqrt{100}}$ = `r round(SEM3,2)`. 
