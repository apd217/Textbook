...
The last chapter ended on a complaint. Correlation tells you that two things move together and how tightly they do it, and then declines to say anything further. It will not tell you *why*, which is a problem for theory. It also will not tell you *how much*, which is a problem for almost everything else.
...
That is the whole upgrade, and it is why the next eight chapters are built on this one.
...
That is what the model predicts. No $e$, because the model does not know your dog died.
...
<!-- ---Alex--- CH11-P01. The old bullet list said "Y = predicted value" under an equation that
contains e, which contradicted the residual section later. Both lines are now shown. The "does
not know your dog died" line is from the review item, not invented here. -->
...
That block is the most-stared-at object in this course. You will meet it in every remaining chapter, it will grow extra rows, and at 2 a.m. before an assignment it will look like a ransom note. So here is what each region is, top to bottom.
...
- **t value** is Estimate divided by Std. Error. That is the one-sample t-test logic, wearing a different hat: how many standard errors is this estimate away from zero?
- **Pr(>|t|)** is that t's p-value, testing the null that this coefficient is really zero.
...
<!-- ---Alex--- CH11-U02, all new. Two things to check: (1) the mean-only comparison is doing
double duty on purpose, it sets up R-squared two sections later so the reader meets 1 - 220/346
having already seen both numbers; (2) I called squaring "a genuine design decision and not a
neutral one" as a quiet forward pointer to outliers and influence in the diagnostics chapter.
Cut that clause if it feels like too much for Week 5. -->
...
Nonlinear transformations are a different animal. Logs, square roots, and powers change the model itself, along with $t$, $p$, and what the slope even means. The correlation chapter showed this the hard way with ice-cream⁴, where a perfectly good relationship fell apart the moment we bent the scale.
...
- That $\beta$ of `r apa.dec(beta)` should look familiar. It is the correlation from the top of this chapter, which is the payoff we proved a section ago. In simple regression you can report either, but never present them as two separate findings.
...
<!-- ---Alex--- CH11-U03, all new, and the piece I would most like your eyes on since it is
the template every later chapter copies. Two calls I made: (1) the beta is recomputed inline
from a standardized model rather than reusing std.reg from the section above, so this block
stands alone if a reader jumps straight here from the homework; (2) I added the p < .001
callout because the drafted write-up hard-codes that string, and students copy example
write-ups verbatim. Cut it if it is one warning too many. -->
