...
::: {.callout-note title="Same Two IVs, Same One DV, Still Nobody Measured Twice"}
...
::: {.callout-warning title="Why We Are Allowed to Do This, and You Are Not"}
...
Doing this to *real* continuous data is a different matter. Median splits and extreme-groups designs throw away information you paid to collect, shrink your statistical power, and can manufacture effects that are not there [@maccallum2002]. Never do this or I will chase you with my Nerf bat.  Notice what happens below: 200 participants become 81. We are discarding well over half the study to make a picture that is easier to draw.
...
*Figure 1.* Predicted friendship closeness as a function of agreeableness group and social engagement. Points represent model-estimated cell means; error bars represent 95% confidence intervals. Among students who went out (*Go out*; orange), highly agreeable students formed substantially closer friendships than less agreeable students (*d* = `r d_go`). Among students who stayed in (*Stay in*; blue), agreeableness group was not significantly associated with friendship closeness (*d* = `r d_stay`).

```{r apa-figure, fig.width=5, fig.height=4, echo=TRUE}
#| fig-alt: "A publication-style figure with Agreeableness Group, Low Nice and High Nice, on the x axis and Predicted Friendship Closeness 0 to 100 on the y axis. Two non-parallel lines connect model-estimated cell means with 95 percent confidence error bars. The blue Stay in line is nearly flat from about 16 to 19; the orange Go out line rises steeply from about 23 to about 60. The legend, titled Social Engagement, sits above the plot."
...
F_int   <- round(aov_tab["NiceGroup_:GoingOut_", "F value"], 2)
...
# Type III needs effect coding. Fit a second copy of the same model with contr.sum
# rather than disturbing Model.2, which every coefficient interpretation above depends on.
Model.2.sum <- lm(Closeness ~ NiceGroup_ * GoingOut_, data = Friend.Data,
                  contrasts = list(NiceGroup_ = contr.sum,
                                   GoingOut_  = contr.sum))
carAov_II  <- car::Anova(Model.2, type = 2)
carAov_III <- car::Anova(Model.2.sum, type = 3)
# Index by NAME, never position: Anova(type = 3) inserts an (Intercept) row and
# every positional index silently shifts by one.
F_nice_I   <- round(aov_tab["NiceGroup_", "F value"], 2)
F_go_I     <- round(aov_tab["GoingOut_",  "F value"], 2)
F_nice_II  <- round(carAov_II["NiceGroup_",  "F value"], 2)
F_go_II    <- round(carAov_II["GoingOut_",   "F value"], 2)
F_nice_III <- round(carAov_III["NiceGroup_", "F value"], 2)
F_go_III   <- round(carAov_III["GoingOut_",  "F value"], 2)
F_int_III  <- round(carAov_III["NiceGroup_:GoingOut_", "F value"], 2)

# What Type III returns if you forget contr.sum, for the warning callout
carAov_bad <- car::Anova(Model.2, type = 3)
F_nice_bad <- round(carAov_bad["NiceGroup_", "F value"], 2)
F_go_bad   <- round(carAov_bad["GoingOut_",  "F value"], 2)
t1_sq      <- round(summary(Model.2)$coefficients["NiceGroup_High Nice", "t value"]^2, 2)
t2_sq      <- round(summary(Model.2)$coefficients["GoingOut_Go out", "t value"]^2, 2)
```
...
### The Sums of Squares Fight, and Why Everyone Says "Type III"
...
::: {.callout-warning title="Type III Without Effect Coding Is Nonsense, and R Will Not Warn You"}
...
Run `Anova(model, type = 3)` on a dummy-coded model and R returns a table without complaint. The numbers are simply not main effects. On our data it reports Niceness Group at $F =$ `r F_nice_bad` and Going Out at $F =$ `r F_go_bad`, which look like a null result and a small effect. They are neither. They are the *simple effects* $B_1$ and $B_2$ in disguise: square the $t$ values from the regression output and you get `r t1_sq` and `r t2_sq`, the same two numbers. Type III asked "what does niceness do at zero on the other variable", and with dummy coding zero means the Stay-in group, not the average.
...
::: {.callout-note title="Two Doors We Are Walking Past, On Purpose"}
...
Both live in the advanced contrasts chapter. If you are an undergraduate in 343, you are done here and you may leave happy.
:::
