...
The central idea is simple: turn each pair into one difference score, then ask whether the average difference is zero. That is the whole test. Nobody needs a random effect yet. The random effects are waiting in the a later chapter with lots of paperwork.
:::
...
Measuring someone twice buys you statistical power (easier time to see effect if one actually exists), but it charges you benefit. Three things ride along with that second measurement:
...
- **Fatigue effects.** They get worse on the task because they are tired and would like to go home. *I tried to lock the doors, but Fire Marshal Bill paid me visit.*
...
Neither shape is more correct than the other. They are the same data wearing different outfits, and different tools insist on different outfits:
...
- Mixed models will also want long, for the same reason, when we get there.
