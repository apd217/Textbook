...
```{r simulate data}
set.seed(42)   # 42 is the answer to the everything btw
...
Adding a control variable can also make a slope grow, disappear, reverse, or change statistical significance. Those patterns can reflect confounding, suppression, collinearity, measurement problems, or a poorly chosen control. The [Statistical Control](Ch_15_Statistical_Control.qmd) chapter takes apart what statistical control is actually doing.
...
The next chapter, [Hierarchical Regression](Ch_14_Hierarchical_Regression.qmd), picks up Models 1 and 3 where we just left them and asks whether adding Test Anxiety improved the model by more than chance. After that, [Statistical Control: What Are We Actually Removing?](Ch_15_Statistical_Control.qmd) uses these same students to show exactly how residuals, semi-partial correlation, partial correlation, standardized beta, and change in $R^2$ fit together. No new imaginary undergraduates will be recruited until the current ones have finished the exam.
:::
