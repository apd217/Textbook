...
#| fig-alt: "Twelve points, one for each observation dropped, against a horizontal orange line at the full-sample mean of 45.8. Eleven of them drift gently downward between 48.9 and 44.1, all hugging the line within a few points. The twelfth, labeled as the 190-minute student, sits alone at 32.7, more than ten points below the line and nowhere near the rest."
plot(
...
#| fig-alt: "Histogram of 5,000 bootstrapped medians, x axis running 20 to 50, with a vertical orange line at the observed median of 30. This is not a smooth bell. A spike of about 1,900 resamples lands exactly at 30, a bar of about 1,000 just above it, a smaller cluster around 26 to 28 below, and a separate lumpy group spread across 35 to 40 with a stray bar near 42. The gaps between the clumps are real: the median of 55 whole-number delays can only land on a limited menu of values."
set.seed(345)
B <- 5000
...
The long-run-procedure interpretation applies to bootstrap intervals too, and the evidence says almost nobody holds onto it: see the Standard Error chapter for the depressing Hoekstra et al. numbers [@hoekstra2014]. Reread that callout if the word *confidence* has started whispering probability statements to you.
:::
...
And remember that computation is not forgiveness.
