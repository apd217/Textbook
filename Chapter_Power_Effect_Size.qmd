...
- **First section**: *why* power exists, which comes down to the fact that small samples flatter you. Conceptually the hardest part, and not the part your exam asks about.
- **The middle**: the vocabulary that is on the exam (Type I and Type II error, $\alpha$, $\beta$, power) and how to compute power in R. If you are short on time, start at "Type I and Type II Error" and read forward.
- **Advanced Topics at the end**: for when someone you love runs a pilot study. Genuinely optional.
:::
...
Two different things are wobbling here, and they are worth keeping apart, because this is exactly the two-counts trap the standard error chapter warned about.
...
Cohen borrowed the idea from signal detection theory, developed for radar operators in World War II who had to decide whether that blip was a plane or noise. The [one-sample t-test chapter](Chapter_OneSample_t_test.qmd) built $d$ out of those two curves and drew the pictures, so we do not need to do that again. The one-line version: $d$ is the distance between the noise distribution and the signal-plus-noise distribution, measured in standard deviations.

What that chapter did not point out is that signal detection theory hands us this chapter's 2×2 for free. Here is the hearing-test version of the table above, and it is the *same table*:
