...
<!-- ---Alex--- Cold open (VOICE_GUIDE 15/17), which you approved on 2026-08-17.
The chapter previously opened on "It Depends, Again" plus a Fast Review recap,
which is the same first-position-recap failure as Ch10/15/16. The simulation
chunk moved up here so the demonstration can happen before any definition; the
old "Simulating Sleep, Caffeine, and Questionable Judgment" heading is now
"Does the Interaction Earn Its Place?" and keeps the additive-vs-interaction
comparison, since it no longer simulates anything. Every number below was run
first. The sign flip and the three identities are real and not tuned. -->
