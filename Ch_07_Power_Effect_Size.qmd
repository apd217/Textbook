...
- **Two separate groups**: the [independent-samples chapter](Ch_08_Independent_t.qmd) closes with `type = "two.sample"`, where `n` means *per group*, plus Hedges' correction for the bias small samples put into $d$.
- **The same people twice**: the [paired-samples chapter](Ch_09_Paired_t.qmd) closes with `type = "paired"`, and shows how much cheaper a repeated-measures design is for the same effect.
