...
<!-- ---Alex--- New cold open, replacing the recap that used to sit at the top. The
     old "Where We Are and Where We Are Going" section is still here, just demoted
     below it, which is the same shape Ch15 ended up with. The demonstration is the
     two separate regressions the chapter already described in the abstract further
     down ("Imagine, for a moment..."); making it real up front means the abstract
     version could be cut, and B3 becomes a payoff instead of a new fact. -->

## Two Slopes, and Nothing That Compares Them

Here is a study you could run tomorrow. Follow 200 first-semester students. Have a trained panel rate how warm and agreeable each one is, 0 to 10. Note whether they actually went out to parties and social events, or stayed home all semester. At the end of the term, ask each student how close a friendship they formed with one particular person they met in week one, on a 0 to 100 thermometer.

Then split them into the ones who went out and the ones who stayed in, and run an ordinary regression in each half predicting closeness from niceness. Two groups, two regressions, nothing you have not already done a dozen times.

```{r cold-open-two-regressions, echo=TRUE}
...
Now defend it. A reviewer asks the one question anybody would ask: are those two slopes *actually* different, or did two numbers just bounce apart the way numbers do? Go find that test in the output above.

It is not there. You have two intercepts, two slopes, four standard errors and four *p*-values, and every single one of them is a statement about one group considered entirely on its own. The first table has never heard of the second table. The comparison the whole study was built to make is the one comparison neither regression performed, and no amount of staring at `0.35` and `<.001` will conjure it, because a small *p*-value and a large one are not a test of the distance between them.
...
*You cannot test a difference with two analyses that have never met.*
...
#| fig-alt: "A publication-style figure with Agreeableness Rating 0 to 10 on the x axis and Predicted Friendship Closeness 0 to 100 on the y axis. Two non-parallel prediction lines with shaded 95 percent confidence bands, over faint raw data points. The blue Stay in line is essentially flat near 15 to 17 across the full range. The orange Go out line rises steeply from about 16 at the left to about 61 at the right. The legend, titled Social Engagement, sits above the plot."
