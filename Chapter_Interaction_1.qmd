...
p_A <- ggplot(hyp_A, aes(x = BeingNice, y = Predicted,
  ...
  labs(title = "Hypothesis A: No Interaction",
       ...
       x = "Being Nice (0-10)", y = "Predicted New Friends",
       color = NULL) +
  theme_bw() +
...
Imagine, for a moment, that instead of running one regression on the full dataset, you split the data in half: one dataset with only the students who *stay in*, and one with only the students who *go out*. You then run a separate simple regression (Niceness → Friends) in each half.
...
Friend.Data <- data.frame(BeingNice = X, GoingOut = Z, NewFriends = Y)
Friend.Data$GoingOut_ <- factor(Friend.Data$GoingOut,
                                levels = c(0, 1),
                                labels = c("Stay in", "Go out"))
Friend.Data$BeingNice.C <- scale(Friend.Data$BeingNice,
...
So when we center *Being Nice* (which we will do before running the models), a centered value of 0 will correspond to a person who scored approximately `r round(mean(Friend.Data$BeingNice), 2)` on the original 0–10 scale.
...
GGally::ggpairs(
  Friend.Data[, c("BeingNice", "NewFriends", "GoingOut_")],
  lower = list(continuous = "smooth", combo = "box_no_facet"),
  diag  = list(continuous = "barDiag"),
  columnLabels = c("Being Nice", "New Friends", "Going Out")
) + theme_bw()
```
...
- **Intercept:** The predicted number of new friends for a person of *average niceness* (BeingNice.C = 0, i.e., roughly a `r round(mean(Friend.Data$BeingNice), 1)` on the original scale) who *stays in* (GoingOut = reference group). This is a real, interpretable prediction: someone who is neither particularly nice nor antisocial, and who mostly stays home.

- **BeingNice.C:** Controlling for whether the student goes out or not, each additional point of niceness (above the average) is associated with about `r round(coef(Model.1)["BeingNice.C"], 2)` more friends. This is averaged across both groups.

- **GoingOut_Go out:** Controlling for how nice the student is, going out is associated with about `r round(coef(Model.1)["GoingOut_Go out"], 1)` more friends than staying in. This is the overall main effect of social engagement.
...
So the intercept is the predicted number of new friends for **an average-niceness person who stays in**. Looking at our model output, the intercept is approximately `r round(coef(Model.2)[1], 1)`.
...
On the plot, this is the vertical gap between the two lines at the center of the x-axis (average niceness). Students who go out have about `r round(coef(Model.2)["GoingOut_Go out"], 0)` more close friends than students who stay in, when both groups are at average niceness. This gap is highly significant.
...
|---|---|---|---|
| **Intercept** | `r round(coef(Model.2)[1], 2)` | Predicted friends for average-niceness, Stay-in student | Is this > 0? |
| **BeingNice.C** | `r round(coef(Model.2)["BeingNice.C"], 2)` | Slope of Being Nice *for Stay-in group* (simple effect) | Is the Stay-in slope ≠ 0? |
...
- **Go out group:** The slope of Being Nice is approximately `r b_go`, which is *significantly* positive ($p <$ .001). For students who go out, every additional point of niceness is associated with about `r round(b_go, 0)` more close friends.
...
m_stay  <- round(mean(Friend.Data$NewFriends[Friend.Data$GoingOut_ == "Stay in"]), 1)
sd_stay <- round(sd(Friend.Data$NewFriends[Friend.Data$GoingOut_  == "Stay in"]), 1)
m_go    <- round(mean(Friend.Data$NewFriends[Friend.Data$GoingOut_ == "Go out"]), 1)
sd_go   <- round(sd(Friend.Data$NewFriends[Friend.Data$GoingOut_  == "Go out"]), 1)
```

Prior to the main analysis, descriptive statistics were examined by group. Students who stayed in formed fewer close friendships on average (*M* = `r m_stay`, *SD* = `r sd_stay`) compared with students who went out regularly (*M* = `r m_go`, *SD* = `r sd_go`).
