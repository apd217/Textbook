...
What, it is not there? Weird. You have two intercepts, two slopes, four standard errors and four *p*-values, and every single one of them is a statement about one group considered entirely on its own. You drew an inference based on two separate tables that have nothing to do with each other. You inferred, because the stay-in slope was smaller than the go-out slope, that being nice clearly does more for the people who go out. The comparison the whole study was built to make was never performed, and no amount of staring at `p = .35` and `p < .001` will conjure it, because a small *p*-value next to a large one is not a test of the difference between them. <!-- ---Alex--- Your sentence here read "You infered cause the slope of stay in group was < go out group of Being Nice scale (0-10) that clearly those go our are nicer people." I fixed the spelling, but I also changed the *claim*, so please check I read you right. As written it said the reader concluded the go-out students **are nicer people**, which is a claim about group means on X. The inference the cold open actually sets up, two paragraphs earlier, is "being nice only pays off if you leave the house", which is a claim about the two **slopes**. I went with the slopes reading, since that is the one the missing test would have tested. Revert me if you meant the other. -->  *You cannot test a difference with two analyses that have never met.*
