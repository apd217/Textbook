...
#| fig-cap: "All three variables against each other. Read it the way we read the matrix in the correlation chapter: diagonal is each variable alone, below is the scatterplot, above is the number."
#| fig-alt: "A three-by-three matrix comparing study hours, anxiety, and exam score. The diagonal shows each variable's density curve, the lower triangle shows scatterplots, and the upper triangle shows correlations with significance stars."
GGally::ggpairs(
  dat,
...
#| fig-alt: "Three overlapping circles labelled Study Hours, Test Anxiety, and Exam Score. The overlaps between each predictor and the outcome are labelled a and b, and the region where all three overlap is labelled c."
#| echo: false
#| fig-width: 6.5
#| fig-height: 5.5
...
This student would be predicted to score about `r round(predicted, 1)` on the exam.
...
::: {.callout-warning title="If Region c Comes Out Negative, You Found Something"}
...
You did not. A negative $c$ means the two predictors are doing something called **suppression**, where adding one predictor makes the other's contribution *larger* rather than smaller. It is real, it is not rare, and it is the point at which the Venn diagram metaphor quietly resigns and goes home. The next chapter takes suppression apart properly. Grad readers who want the full treatment should see Cohen, Cohen, West and Aiken, chapter 3.
:::
...
```{r plot study before, echo=TRUE, fig.width=9, fig.height=3.6}
#| fig-cap: "The Study Hours slope, before and after controlling for Anxiety. Same y-axis on both panels, so the difference you see is real."
#| fig-alt: "Two line plots side by side, both showing predicted exam score rising with study hours. The right panel, which controls for anxiety, rises slightly less steeply than the left."
library(patchwork)
...
So the controlled slope is the answer to a different question, and usually a more useful one. That is not the same as being a better-estimated answer, and it is definitely not the same as being "truer."

<!-- ---Alex--- CH13-P01. The old sentence said the controlled slope "is more precise," which
means the opposite of the truth: controlling routinely inflates the SE, and the diagnostics
chapter's VIF section is entirely about that. Worth a look since the replacement is longer than
the original and this chapter is already your most carefully hedged one. -->


### The Anxiety Slope — Before and After Controlling

```{r plot anxiety before, echo=TRUE, fig.width=9, fig.height=3.6}
#| fig-cap: "The Anxiety slope, before and after controlling for Study Hours. Same y-axis again."
#| fig-alt: "Two line plots side by side, both showing predicted exam score falling as anxiety rises. The right panel, which controls for study hours, falls slightly less steeply than the left."
...
```{r plot full model, echo=TRUE, fig.width=5, fig.height=4}
#| fig-cap: "Predicted exam score by study hours, drawn separately for low, average, and high anxiety. The lines are parallel, and that is the point."
#| fig-alt: "Three parallel upward-sloping lines showing predicted exam score against study hours, one line each for low, average, and high anxiety. The lines are stacked vertically and never cross."
plot_model(
  Model.3,
  type         = "pred",
...
Adding a control variable can also make a slope grow, disappear, reverse, or change statistical significance. Those patterns can reflect confounding, suppression, collinearity, measurement problems, or a poorly chosen control. The [Statistical Control](Chapter_Control.qmd) chapter takes apart what statistical control is actually doing.
...
::: {.callout-tip title="Next: Did the Second Predictor Earn Its Keep?"}
The next chapter, [Hierarchical Regression](Chapter_Hierarchical_Regression.qmd), picks up Models 1 and 3 where we just left them and asks whether adding Test Anxiety improved the model by more than chance. After that, [Statistical Control: What Are We Actually Removing?](Chapter_Control.qmd) uses these same students to show exactly how residuals, semi-partial correlation, partial correlation, standardized beta, and change in $R^2$ fit together. No new imaginary undergraduates will be recruited until the current ones have finished the exam.
:::
