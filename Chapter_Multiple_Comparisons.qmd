...
In this book's pipeline, multiplicity arrives in exactly one place: `pairs(emmeans(...))` after a model with a categorical predictor. The [categorical predictors chapter](Chapter_Regression%20with%20Cat%20Variables.qmd) left a promise here. It ran three unadjusted comparisons behind a significant omnibus *F*, which is Fisher's protected LSD, and it warned that the protection expires at exactly three groups. So let us arrive with four.
...
- **Exactly three groups behind a significant omnibus *F*?** `adjust = "none"` is defensible, and only there. That is the protected LSD case from the [categorical predictors chapter](Chapter_Regression%20with%20Cat%20Variables.qmd). At four groups, as above, it is the inflation problem wearing a lab coat.
- **Screening a large pile of outcomes to decide what to study next?** `adjust = "fdr"`, and call the results exploratory, because that is what you have promised. See the next section for what FDR is actually good at.
