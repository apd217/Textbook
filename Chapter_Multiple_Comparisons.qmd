...
Note also that Tukey and Holm disagree about *how* dead it is. Both control the familywise error rate over the six comparisons, and neither is uniformly the more powerful of the two; they distribute their power differently across contrasts. Choosing between them on the basis of which one gives you a smaller number for the comparison you like is not a choice. It is the same problem one level up.

::: {.callout-tip title="Which `adjust=` Do I Want?"}
- **All pairwise comparisons after an omnibus test?** Leave the default. Tukey was designed for exactly this family and `emmeans` is already applying it. Say so in the write-up.
- **A few specific contrasts your theory named in advance?** `adjust = "holm"`. The family is your planned contrasts, not every pair the software can generate, and Holm is never less powerful than Bonferroni.
- **Exactly three groups behind a significant omnibus *F*?** `adjust = "none"` is defensible, and only there. That is the protected LSD case from the [categorical predictors chapter](Chapter_Regression%20with%20Cat%20Variables.qmd). At four groups, as above, it is just the inflation problem wearing a lab coat.
- **Screening many outcomes to decide what to study next?** `adjust = "fdr"`, and describe the results as exploratory, because that is what you have promised.

Whatever you pick, name it and name the family in the write-up. "Pairwise comparisons were Tukey-adjusted across all six condition pairs" is one clause, and it is the difference between a reader trusting the table and a reader reconstructing it.
:::
