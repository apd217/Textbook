...
That last option is the one you will actually reach for, and it deserves more than a name. "Robust standard errors" and "the sandwich estimator" get used as incantations by people who could not say what is being made robust to what, so here is the machinery.
...
So why did the standard errors grow? Participant 60, and now you have the formula to see exactly how.
...
| **`lmtest`** | `coeftest()` and `bptest()`, re-testing a model against a given covariance matrix | Zeileis and Hothorn's diagnostic-checking toolkit [@zeileis2002] |
