...
<!-- ALEX: this is the PPTX pirate bit rebuilt as a live chunk, so the r is computed from the numbers on the page rather than typed. Three things to rule on: (1) the "I should probably admit" paragraph is me being honest that the pirate data is fake -- cut it if you'd rather let the gag run unqualified; (2) "It has never been outside" is my line, not yours, so kill it if it clunks; (3) the heading is a joke-first heading, which VOICE_GUIDE 4 says not to do because the TOC has to work as navigation. I think it earns the exception since it IS the chapter's thesis, but it's your call. Safe alternative: "Why Two Variables at Once, and Why Pirates". -->
...
<!-- ALEX: this section is new. It exists because ice cream and happiness previously appeared for the first time as colnames() with no introduction, which is where the chapter felt most like slides. It also plants the callback that the r^2 section pays off ("heat may produce both while quietly stealing credit"). -->
...
This is worth proving rather than asserting, so the next four sections do the same analysis four times while abusing the measurement scale in a different way each time. Watch the reported $r$ in the corner of each plot. Three of the four will not move.

<!-- ALEX: added this lead-in because the four "Scale change:" sections previously arrived as four bare plots with no framing, and a reader has no idea they are watching one demonstration in four parts. -->
...
Groups that do something stranger than flatten, and reverse a trend outright, are Simpson's paradox. That one waits for the multiple regression chapters.
...
<!-- ALEX: I kept your original paragraph verbatim and added two after it. The first introduces the unmeasured third variable (heat), which your Short Story already pays off at the end of the chapter with "heat may produce both while quietly stealing credit" -- previously that line arrived with no setup. The second closes the pirate loop. -->
...
This is also where Meehl's crud factor stops being a quotation and starts being a picture. Look at how few of these cells are anywhere near zero. In real data they rarely are, and a matrix like this is where most fishing expeditions begin: run enough pairs and something will look publishable.
